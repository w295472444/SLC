...
#if defined(NDEBUG)
# error "Starlightcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Starlightcoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Starlightcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Starlightcoin: find block wanted by given orphan block
...
{
    int64_t nSubsidy = (nCoinAge  * 0.01 * COIN) / 365;
    return nSubsidy + nFees;
}

static const int64_t nTargetTimespan = 16 * 60;

// Starlightcoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Starlightcoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Starlightcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Starlightcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Starlightcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Starlightcoin: track money supply and mint amount info
...
// Starlightcoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Starlightcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Starlightcoin: compute stake entropy bit for stake modifier
    ...
    // Starlightcoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Starlightcoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Starlightcoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Starlightcoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Starlightcoin-loadblk");
